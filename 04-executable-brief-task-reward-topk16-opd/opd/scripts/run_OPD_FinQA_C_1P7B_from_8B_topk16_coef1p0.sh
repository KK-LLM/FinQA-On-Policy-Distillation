...
CACHE_ROOT=${CACHE_ROOT:-/root/autodl-tmp/opd_finqa_c/cache}

# Complete merged Hugging Face models, not adapter-only checkpoint directories.
export STUDENT_MODEL_PATH=${STUDENT_MODEL_PATH:-"${MODEL_ROOT}/Qwen3-1.7B-LoRA-BPA-C-0826-checkpoint-7920"}
export TEACHER_MODEL_PATH=${TEACHER_MODEL_PATH:-"${MODEL_ROOT}/Qwen3-8B-LoRA-BPA-C-0826-checkpoint-14800"}
export FINQA_TOKENIZER_PATH=${FINQA_TOKENIZER_PATH:-"${STUDENT_MODEL_PATH}"}
export TRAIN_FILE=${TRAIN_FILE:-"${DATA_ROOT}/train_finqa_c_scheme_a_0828.parquet"}
export VAL_FILE=${VAL_FILE:-"${DATA_ROOT}/valid_finqa_c_scheme_a_0828.parquet"}
...
export MAX_PROMPT_LENGTH=${MAX_PROMPT_LENGTH:-5450}
# Gold three-field responses use at most 247 Qwen3 tokens; 384 leaves rollout headroom.
export MAX_RESP_LENGTH=${MAX_RESP_LENGTH:-384}
...
# floor(3422 / 96) = 35 outer steps/epoch, about 140 total steps.
# Keep the established 9-step save/validation cadence for comparable checkpoints.
...
export PROJECT_NAME=${PROJECT_NAME:-FinQA-C-OPD}
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-finqa_c_qwen3_1p7b_7920_from_qwen3_8b_14800_scheme_a_opd_topk16_taskreward_coef1p0_lr1e5_b96_m24_ppo1_n4_e4}
...
for reward_file in \
    ...
    finqa_three_field_core_c_0828.py; do
    [[ -f ${SCRIPT_DIR}/${reward_file} ]] || {
        echo "Missing Reward file: ${SCRIPT_DIR}/${reward_file}" >&2
        exit 1
    }
done
...
[[ ${LR_SCHEDULER_TYPE} == cosine ]] || {
    echo "This package requires LR_SCHEDULER_TYPE=cosine." >&2
    exit 1
}
[[ ${VAL_DO_SAMPLE} == False ]] || {
    echo "This package requires deterministic validation with VAL_DO_SAMPLE=False." >&2
    exit 1
}
...
printf '%s\n' \
    ...
    "Lengths: prompt/${MAX_PROMPT_LENGTH}, response/${MAX_RESP_LENGTH}" \
...
# Fail before Ray/VERL startup if the real experiment tokenizer cannot load.
if [[ ${OPD_DRY_RUN:-0} != 1 ]]; then
    PYTHONPATH="${SCRIPT_DIR}:${PYTHONPATH:-}" python3 -c \
        'from finqa_three_field_reward_c_0828 import preload_tokenizer; preload_tokenizer()'
fi

exec bash "${SCRIPT_DIR}/on_policy_distillation_gpu_c.sh" "$@"
